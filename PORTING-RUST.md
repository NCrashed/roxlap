...
| **R10** | wasm SIMD via `core::arch::wasm32`; web host (canvas + js glue) as a separate crate. Sub-substages R10.0..R10.5 + locked decisions + risk analysis in [PORTING-WASM.md](PORTING-WASM.md). | **R10.0..R10.4 landed**. R10.0: target-conditional `rayon` with sequential wasm fallbacks; flake ships `lld`. R10.1: wasm-bindgen-test north-pose runs under Node. R10.2: new `roxlap-web` (cdylib) crate — wasm-bindgen `start()` + RAF loop + WASD + pointer-lock mouse-look. R10.3: wasm SIMD 4-pixel batches in `hrend` and `vrend` using `core::arch::wasm32::f32x4_*` simd128 intrinsics; `.cargo/config.toml` adds `target-feature=+simd128`; release wasm has 761 SIMD ops (2× `f32x4.sqrt` + 2× `f32x4.div` from the batches + rustc auto-vectorization). R10.4: roxlap-oracle's render helpers (POSES, render_pose, fnv1a64, lighting bake) extracted into `src/lib.rs` so the bin and the wasm test share one render path; new `tests/wasm_render.rs` renders all 12 poses under Node and diffs against `tests/wasm-hashes.txt` (frozen 2026-05-06). Wasm goldens diverge from x86_64 / aarch64 by design — full-precision wasm SIMD vs SSE/NEON's rsqrt approximations. R10.5 (browser bench + README) not started. | Browser perf benchmark; own wasm goldens (`crates/roxlap-oracle/tests/wasm-hashes.txt`) — frozen 12/12; wasm-bindgen-test passes in CI matrix. |
