...
| **R10** | wasm SIMD via `core::arch::wasm32`; web host (canvas + js glue) as a separate crate. Sub-substages R10.0..R10.5 + locked decisions + risk analysis in [PORTING-WASM.md](PORTING-WASM.md). | **R10.0 + R10.1 + R10.2 landed**. R10.0: target-conditional `rayon` with sequential wasm fallbacks; flake ships `lld`. R10.1: wasm-bindgen-test north-pose runs under Node, hash `409ae24871dfe474`. R10.2: new `roxlap-web` (cdylib) crate — wasm-bindgen `start()` parses the embedded oracle world, drives a `requestAnimationFrame` loop, blits ImageData onto a `<canvas>`. WASD + Space/Shift movement + pointer-lock mouse-look via raw web_sys event handlers. `crates/roxlap-web/index.html` + `Trunk.toml`; flake adds `pkgs.trunk`. `trunk build` from `crates/roxlap-web/` emits a ~760 KB wasm + 18 KB JS bundle. R10.3 (wasm SIMD), R10.4 (wasm goldens), R10.5 (browser bench + README) not started. | Browser perf benchmark; own wasm goldens (`crates/roxlap-oracle/tests/wasm-hashes.txt`); wasm-bindgen-test passes in CI matrix. |
