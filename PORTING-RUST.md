...
| **R10** | wasm SIMD via `core::arch::wasm32`; web host (canvas + js glue) as a separate crate. Sub-substages R10.0..R10.5 + locked decisions + risk analysis in [PORTING-WASM.md](PORTING-WASM.md). | **R10.0..R10.3 landed**. R10.0: target-conditional `rayon` with sequential wasm fallbacks; flake ships `lld`. R10.1: wasm-bindgen-test north-pose runs under Node, hash `409ae24871dfe474`. R10.2: new `roxlap-web` (cdylib) crate — wasm-bindgen `start()` + RAF loop + WASD + pointer-lock mouse-look. R10.3: wasm SIMD 4-pixel batches in `hrend` and `vrend` (mirror of x86_64 SSE / aarch64 NEON paths) using `core::arch::wasm32::f32x4_*` simd128 intrinsics. `1.0 / sqrt(x)` substitutes for `_mm_rsqrt_ps` / `vrsqrteq_f32` since wasm SIMD has no rsqrt approximation; output is bit-identical to the wasm scalar tail (full-precision ops). `.cargo/config.toml` adds `target-feature=+simd128`; release wasm contains 761 SIMD instructions (incl. 2 `f32x4.sqrt` + 2 `f32x4.div` from the two batches plus rustc auto-vectorization on the rest). Trunk release bundle drops to ~362 KB after wasm-opt. `drawboundcubesse` keeps R9.3's scalar non-x86_64 fallback. R10.4 (wasm goldens), R10.5 (browser bench + README) not started. | Browser perf benchmark; own wasm goldens (`crates/roxlap-oracle/tests/wasm-hashes.txt`); wasm-bindgen-test passes in CI matrix. |
