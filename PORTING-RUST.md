...
| **R10** | wasm SIMD via `core::arch::wasm32`; web host (canvas + js glue) as a separate crate. Sub-substages R10.0..R10.5 + locked decisions + risk analysis in [PORTING-WASM.md](PORTING-WASM.md). | **R10.0 landed**: `rayon` is target-conditional, three parallel call-sites (`opticast::run_strip_parallel`, `world_lighting::update_lighting`, `sprite::draw_sprites_parallel`) have sequential `#[cfg(target_arch = "wasm32")]` fallbacks, `flake.nix` ships `lld` for the wasm linker. `cargo build --target wasm32-unknown-unknown --workspace --exclude roxlap-host --exclude roxlap-cave-demo` green; native `cargo test --workspace` 363/363. R10.1..R10.5 not started. | Browser perf benchmark; own wasm goldens (`crates/roxlap-oracle/tests/wasm-hashes.txt`); wasm-bindgen-test passes in CI matrix. |
