...
| 6 | **Lighting: pre-baked alpha bytes from the CPU bake.** Same as the CPU path. No runtime lighting on GPU in v1. | Single-pass shader. Emissive / dynamic light deferred. **[STALE since DL/SL/XS/EV: the GPU shaders now carry full dynamic lighting (sun + point/spot, hard shadows, cross-grid + sprite shadows) and emissive at CPU parity — see PORTING-DYNLIGHT.md. Kept as the historical v1 record.]** |
