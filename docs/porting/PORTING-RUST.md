...
| **R9** | ARM NEON via `core::arch::aarch64`; macOS arm64 + Linux aarch64 in CI. | **landed** (R9.1..R9.4, merged from `r9-neon` on 2026-05-05). R9.1–R9.2: NEON 4-pixel `rsqrtps` batches in `hrend` / `vrend` via `vrsqrteq_f32` + one Newton–Raphson step (`vrsqrtsq_f32`) for ~16-bit z-buffer precision; NEON is baseline on AArch64 so no runtime feature check. R9.3: scalar `drawboundcubesse` port replaces the non-x86_64 no-op stub; uses IEEE 754 `1.0 / z` instead of SSE2's `_mm_rcp_ps` for perspective projection (±1px AABB drift on sprite poses). R9.4: per-arch goldens at `tests/golden-hashes-aarch64.txt` — 8 of 12 poses bit-exact with x86_64 (all non-sprite poses match; 4 sprite poses differ from x86_64 by sub-pixel rcp precision). | `cargo run -p roxlap-oracle -- diff --golden tests/golden-hashes-aarch64.txt` reports 12 match on aarch64; framebuffer hashes unchanged on x86_64. Apple Silicon bench shows ~0.5% mean uplift (radar gather, not rsqrt math, is the bottleneck). |
| **R10** | wasm SIMD via `core::arch::wasm32`; web host (canvas + js glue) as a separate crate. Sub-substages R10.0..R10.5 + locked decisions + risk analysis in [PORTING-WASM.md](PORTING-WASM.md). | **landed** (R10.0..R10.5). R10.0: target-conditional `rayon` with sequential wasm fallbacks; flake ships `lld`. R10.1: wasm-bindgen-test north-pose runs under Node. R10.2: new `roxlap-web` (cdylib) crate — wasm-bindgen `start()` + RAF loop + WASD + pointer-lock mouse-look. R10.3: wasm SIMD 4-pixel batches in `hrend` and `vrend` (`core::arch::wasm32::f32x4_*`); release wasm has 761 SIMD ops. R10.4: oracle render helpers extracted into `src/lib.rs`; `tests/wasm_render.rs` diffs all 12 poses against `tests/wasm-hashes.txt` (frozen 2026-05-06). R10.5: `B` keybind in `roxlap-web` runs a 300-frame in-browser bench dumping min/p50/mean/p99/max ms + fps to console; per-crate README documents install / serve / bench / bundle size / browser support; root README's crate table + status updated. Trunk release bundle ~360 KB wasm + ~14 KB JS shim. | Browser perf benchmark; own wasm goldens (`crates/roxlap-oracle/tests/wasm-hashes.txt`) — frozen 12/12; wasm-bindgen-test passes in CI matrix. |
| **R11** | Polish: docs, examples, version 0.1 publish to crates.io. Bundled CD.1..CD.9 (voxel-edit API + procedural cave demo) per the locked 2026-05-02 plan: 0.1.0 ships an operational engine, not just a renderer. | **landed (2026-05-07)**. R11.0..R11.8: 11 rustdoc warnings cleared, public-API surface trimmed, Cargo metadata + workspace inheritance for `keywords` / `categories` / `documentation`, 2 examples (`oracle_view`, `parse_kv6`), CHANGELOG, doc tests. CD.1..CD.9: `roxlap-formats::edit` (delslab / insslab / scum2 / setspans / setcube / setsphere / setrect with byte-fixture parity vs voxlap C), new `roxlap-cavegen` crate (hand-rolled Worley + Perlin), new `roxlap-cave-demo` bin (winit + softbuffer + bullets + carve + per-axis collision + fog + F/R toggles). R11.9: `roxlap-formats v0.1.0`, `roxlap-core v0.1.0`, `roxlap-cavegen v0.1.0` published to crates.io. | https://crates.io/crates/roxlap-{formats,core,cavegen} live; `cargo install roxlap-cave-demo` will fly through a procedural cave with WASD + LMB-fire; 363 workspace tests + 12 oracle goldens green. |
