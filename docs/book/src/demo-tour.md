...
| **Boarding** | The CA/OC/FW composition in third person: a walking character (stairwells, jumps, auto-swims the flooded bilge), the deck clip following the deck they're on, walls melting inside the keyhole (`FrameParams::view_cutout`), and fog of war restricting the view to what the character has seen. `K` keyhole · `V` deck-follow/manual · `C` shoulder/tele-iso · `F` fog of war · `G` fog light gate · wheel resizes. | [4](rendering.md) |
...
| `ROXLAP_BOARD_DECK=0..=2` | Boarding scene: spawn the character on that deck (2 = the flooded bilge) |
